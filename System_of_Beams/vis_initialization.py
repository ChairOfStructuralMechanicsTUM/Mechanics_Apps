...
def configure_input_plot(p, div_xy, max_indep_elements):
    ...
    p.circle('x', 'y', source=dat_src.ds_input, radius=dat_src.catch_radius, alpha=0.0,
             color=coL.HtmlColors.BLUE_TUM.value, nonselection_alpha=0.0)
    # tooltips info of nodedependent image glyph elements
    p.circle(x='glyph_x', y='glyph_y', source=dat_src.ds_glyph_springsPointMomentTemp,
             radius=dat_src.catch_radius, alpha=0.0, color=coL.HtmlColors.GREEN.value, nonselection_alpha=0.0)
    # show the fist chosen node for a nodedependent element needing two existing nodes
    p.circle(x='node_x', y='node_y', source=dat_src.ds_1st_chosen, radius=dat_src.catch_radius,
             color=coL.HtmlColors.RED.value, fill_alpha=0.3, line_alpha=1.0)
    # glyphs for beams/ rods and tooltips info about them
    p.rect(x='x', y='y', source=dat_src.ds_glyph_beam, angle="angle", width="width", height=0.1,
    ...
    p.patches(xs='patch_x', ys='patch_y', source=dat_src.ds_glyph_lineload, fill_alpha=0.3, line_alpha=1.0,
              color=col_lineloads, nonselection_alpha=0.3)  # NaN values to plot multiple polygons in one patch
    # arrows for line loads
    p.multi_line(xs='xs', ys='ys', source=dat_src.ds_arrow_lineload, color=col_lineloads, line_alpha=1.0, line_width=3)

    # JavaScript callback to create an element when mouse tap in plot is detected and a button element is activated
    p.js_on_event(Tap, vis_cbs.cb_plot_tap(doc.div_input, max_indep_elements))
    p.js_on_event(Tap, vis_cbs.cb_plot_xy(div_xy))

    # callback to react when the data source of the input plot has changed
    dat_src.ds_input.on_change('data', vis_cbs.cb_adapt_plot_indep)

    # callback to react when the data source for node-dependent elements has changed
    dat_src.ds_chosen_node.on_change('data', vis_cbs.cb_adapt_plot_nodedep)

    # callback to react when the data source for showing element info has changed
    dat_src.ds_element_info.on_change('data', vis_cbs.cb_show_element_info)

    # JavaScript callback to react when objects of the plot datasource change their 'selected' status
    dat_src.ds_input.selected.js_on_change('indices', vis_cbs.cb_get_selected(doc.div_input))
    dat_src.ds_input_selected.on_change('data', vis_cbs.cb_show_selected)
...
def expand_msg2user(msg, bg_color="white"):
    ...
    """
    # global doc # div_msg, msg2user
    doc = glob_var.doc

    if bg_color == "red":
        doc.div_msg.css_classes = ["MSG_BOX_RED"]
    elif bg_color == "orange":
        doc.div_msg.css_classes = ["MSG_BOX_ORANGE"]
    else:
        doc.div_msg.css_classes = ["MSG_BOX"]

    doc.msg2user += "<br/>"
    doc.msg2user += msg
    doc.div_msg.text = "<ul>" + doc.msg2user + "</ul>"
    return doc.msg2user
...
def initialize(max_indep_elements=20, catch_radius=0.15):
    ...
    """
    # global doc
        # plot_input, buttons, div_msg, div_input, \
    ...
    # doc = doc_el.DocElement(doc_element)
    doc = glob_var.doc
    dat_src = glob_var.DataSources
    dat_src.catch_radius = catch_radius
    ...
    div_input_width = doc.plot_input.plot_width - div_xy.width - 10
    doc.div_input       = Div(width=div_input_width, height=div_xy.height)
    ...
    doc.plot_input.toolbar.logo = None
    configure_input_plot(doc.plot_input, div_xy, max_indep_elements)
    ...
    doc.plot_normal_f       = figure(plot_width=plot_output_width, plot_height=plot_output_height, active_scroll="wheel_zoom")
    doc.plot_normal_f.toolbar.logo      = None
    doc.plot_normal_f.title.text        = 'Normal force'
    doc.plot_normal_disp    = figure(plot_width=plot_output_width, plot_height=plot_output_height, active_scroll="wheel_zoom")
    doc.plot_normal_disp.toolbar.logo   = None
    doc.plot_normal_disp.title.text     = 'Normal displacement'
    doc.plot_shear_f        = figure(plot_width=plot_output_width, plot_height=plot_output_height, active_scroll="wheel_zoom")
    doc.plot_shear_f.toolbar.logo       = None
    doc.plot_shear_f.title.text         = 'Shear force'
    doc.plot_moment         = figure(plot_width=plot_output_width, plot_height=plot_output_height, active_scroll="wheel_zoom")
    doc.plot_moment.toolbar.logo        = None
    doc.plot_moment.title.text          = 'Bending moment'
    doc.plot_shear_disp     = figure(plot_width=plot_output_width, plot_height=plot_output_height, active_scroll="wheel_zoom")
    doc.plot_shear_disp.toolbar.logo    = None
    doc.plot_shear_disp.title.text      = 'Shear displacement'
    doc.plot_shear_angle    = figure(plot_width=plot_output_width, plot_height=plot_output_height, active_scroll="wheel_zoom")
    doc.plot_shear_angle.toolbar.logo   = None
    doc.plot_shear_angle.title.text     = 'Rotation angle'
    doc.plot_list = [doc.plot_normal_f, doc.plot_normal_disp, doc.plot_shear_f, doc.plot_moment, doc.plot_shear_angle, doc.plot_shear_disp]

    # add plot renderer
    ds = ColumnDataSource(data=dict(x=[], y=[]))
    doc.plot_normal_f.      circle('x', 'y', source=ds)
    doc.plot_normal_disp.   circle('x', 'y', source=ds)
    doc.plot_shear_f.       circle('x', 'y', source=ds)
    doc.plot_moment.        circle('x', 'y', source=ds)
    doc.plot_shear_disp.    circle('x', 'y', source=ds)
    doc.plot_normal_f.      circle('x', 'y', source=ds)
    doc.plot_shear_angle.   circle('x', 'y', source=ds)
    ...
    button_support_clamped = Button(label="", css_classes=[eLnum.ElSupEnum.SUPPORT_CLAMPED.name], width=b_height,
                                    height=b_height)
    doc.buttons[eLnum.ElSupEnum.SUPPORT_CLAMPED.value] = button_support_clamped
    ...
    button_support_normal = Button(label="", css_classes=[eLnum.ElSupEnum.SUPPORT_NORMAL_FORCE.name], width=b_height,
                                   height=b_height)
    doc.buttons[eLnum.ElSupEnum.SUPPORT_NORMAL_FORCE.value] = button_support_normal
    ...
    doc.buttons[eLnum.ElSupEnum.SUPPORT_TRANSVERSE_FORCE.value] = button_support_transverse
    ...
    button_support_fixed_conti = Button(label="", css_classes=[eLnum.ElSupEnum.SUPPORT_FIXED_CONTINUOUS.name],
                                        width=b_height, height=b_height)
    doc.buttons[eLnum.ElSupEnum.SUPPORT_FIXED_CONTINUOUS.value] = button_support_fixed_conti
    ...
    button_support_fixed_joint = Button(label="", css_classes=[eLnum.ElSupEnum.SUPPORT_FIXED_JOINT.name],
                                        width=b_height,
                                        height=b_height)
    doc.buttons[eLnum.ElSupEnum.SUPPORT_FIXED_JOINT.value] = button_support_fixed_joint
    ...
    button_support_roller_conti = Button(label="", css_classes=[eLnum.ElSupEnum.SUPPORT_ROLLER_CONTINUOUS.name],
                                         width=b_height, height=b_height)
    doc.buttons[eLnum.ElSupEnum.SUPPORT_ROLLER_CONTINUOUS.value] = button_support_roller_conti
    ...
    button_support_roller_joint = Button(label="", css_classes=[eLnum.ElSupEnum.SUPPORT_ROLLER_JOINT.name],
                                         width=b_height,
                                         height=b_height)
    doc.buttons[eLnum.ElSupEnum.SUPPORT_ROLLER_JOINT.value] = button_support_roller_joint
    ...
    button_spring_support = Button(label="", css_classes=[eLnum.ElSupEnum.SPRING_SUPPORT.name], width=b_height,
                                   height=b_height)
    doc.buttons[eLnum.ElSupEnum.SPRING_SUPPORT.value] = button_spring_support
    ...
    button_spring_moment_support = Button(label="", css_classes=[eLnum.ElSupEnum.SPRING_MOMENT_SUPPORT.name],
                                          width=b_height,
                                          height=b_height)
    doc.buttons[eLnum.ElSupEnum.SPRING_MOMENT_SUPPORT.value] = button_spring_moment_support
    ...
    doc.buttons[eLnum.ElSupEnum.JOINT.value] = button_joint
    ...
    button_joint_normal = Button(label="", css_classes=[eLnum.ElSupEnum.JOINT_NORMAL_FORCE.name], width=b_height,
                                 height=b_height)
    doc.buttons[eLnum.ElSupEnum.JOINT_NORMAL_FORCE.value] = button_joint_normal
    ...
    button_joint_transverse = Button(label="", css_classes=[eLnum.ElSupEnum.JOINT_TRANSVERSE_FORCE.name],
                                     width=b_height, height=b_height)
    doc.buttons[eLnum.ElSupEnum.JOINT_TRANSVERSE_FORCE.value] = button_joint_transverse
    ...
    doc.buttons[eLnum.ElSupEnum.SPRING.value] = button_spring
    ...
    # doc.buttons[eLnum.ElSupEnum.ROD.value] = button_rod
    ...
    doc.buttons[eLnum.ElSupEnum.BEAM.value] = button_beam
    ...
    doc.buttons[eLnum.ElSupEnum.LOAD_POINT.value] = button_load_point
    ...
    button_load_moment = Button(label="", css_classes=[eLnum.ElSupEnum.LOAD_MOMENT.name], width=b_height,
                                height=b_height)
    doc.buttons[eLnum.ElSupEnum.LOAD_MOMENT.value] = button_load_moment
    ...
    button_load_random = Button(label="", css_classes=[eLnum.ElSupEnum.LOAD_LINE.name], width=b_line_width,
                                height=b_height)
    doc.buttons[eLnum.ElSupEnum.LOAD_LINE.value] = button_load_random
    ...
    doc.buttons[eLnum.ElSupEnum.LOAD_TEMP.value] = button_load_temp
    ...
    key_elinfo_n = "name"
    doc.input_element_info[key_elinfo_n] = input_elinfo_name
    ...
    key_elinfo_x = "x"
    doc.input_element_info[key_elinfo_x] = input_elinfo_x
    ...
    key_elinfo_y = "y"
    doc.input_element_info[key_elinfo_y] = input_elinfo_y
    ...
    doc.input_element_info[key_elinfo_angle] = input_elinfo_angle
    ...
    key_elinfo_k = "k"
    doc.input_element_info[key_elinfo_k] = input_elinfo_k
    ...
    doc.input_element_info[key_elinfo_length] = input_elinfo_length
    ...
    key_elinfo_f = "force"
    doc.input_element_info[key_elinfo_f] = input_elinfo_force
    ...
    key_elinfo_m = "moment"
    doc.input_element_info[key_elinfo_m] = input_elinfo_moment
    ...
    key_elinfo_h = "h"
    doc.input_element_info[key_elinfo_h] = input_elinfo_h
    ...
    doc.group_element_info["beam"] = check_elinfo_beam
    ...
    key_elinfo_ea = "ea"
    doc.input_element_info[key_elinfo_ea] = input_elinfo_ea
    ...
    key_elinfo_ei = "ei"
    doc.input_element_info[key_elinfo_ei] = input_elinfo_ei
    ...
    radio_elinfo_ll = RadioGroup(labels=["local", "global"], active=0, disabled=True,  # "angle"
                                 width=elinfo_input_width+elinfo_label_width1)
    doc.group_element_info["ll"] = radio_elinfo_ll
    ...
    doc.input_element_info[key_elinfo_xns] = input_elinfo_xns
    ...
    doc.input_element_info[key_elinfo_xne] = input_elinfo_xne
    ...
    doc.input_element_info[key_elinfo_yqs] = input_elinfo_yqs
    ...
    doc.input_element_info[key_elinfo_yqe] = input_elinfo_yqe
    ...
    key_elinfo_dt = "dT"
    doc.input_element_info[key_elinfo_dt] = input_elinfo_dt
    ...
    key_elinfo_tt = "T"
    doc.input_element_info[key_elinfo_tt] = input_elinfo_tt
    ...
    key_elinfo_at = "aT"
    doc.input_element_info[key_elinfo_at] = input_elinfo_at
    ...
    check_plot_nf.on_change('active', partial(vis_cbs.cb_toggle_characteristic_values, output_plot=doc.plot_normal_f,
                                              div=text_plot_nf))

    check_plot_nd = CheckboxGroup(labels=check_labels, active=check_init_active)
    text_plot_nd = Div(text="", width=100, height=10)
    check_plot_nd.on_change('active', partial(vis_cbs.cb_toggle_characteristic_values, output_plot=doc.plot_normal_disp,
                                              div=text_plot_nd))

    check_plot_sf = CheckboxGroup(labels=check_labels, active=check_init_active)
    text_plot_sf = Div(text="", width=100, height=10)
    check_plot_sf.on_change('active', partial(vis_cbs.cb_toggle_characteristic_values, output_plot=doc.plot_shear_f,
                                              div=text_plot_sf))

    check_plot_mo = CheckboxGroup(labels=check_labels, active=check_init_active)
    text_plot_mo = Div(text="", width=100, height=10)
    check_plot_mo.on_change('active', partial(vis_cbs.cb_toggle_characteristic_values, output_plot=doc.plot_moment,
                                              div=text_plot_mo))

    check_plot_sa = CheckboxGroup(labels=check_labels, active=check_init_active)
    text_plot_sa = Div(text="", width=100, height=10)
    check_plot_sa.on_change('active', partial(vis_cbs.cb_toggle_characteristic_values, output_plot=doc.plot_shear_angle,
                                              div=text_plot_sa))

    check_plot_sd = CheckboxGroup(labels=check_labels, active=check_init_active)
    text_plot_sd = Div(text="", width=100, height=10)
    check_plot_sd.on_change('active', partial(vis_cbs.cb_toggle_characteristic_values, output_plot=doc.plot_shear_disp,
                                              div=text_plot_sf))
    ...
    doc.children_element_info[key_elinfo_n] = elinfo_name
    elinfo_xy = row(text_elinfo_x, input_elinfo_x, spacer_y_a, text_elinfo_y, input_elinfo_y)
    doc.children_element_info[key_elinfo_x] = elinfo_xy
    ...
    doc.children_element_info[key_elinfo_angle] = elinfo_angle_length
    elinfo_k = row(text_elinfo_k1, input_elinfo_k, text_elinfo_k2)
    doc.children_element_info[key_elinfo_k] = elinfo_k
    elinfo_force = row(text_elinfo_force1, input_elinfo_force, text_elinfo_force2)
    doc.children_element_info[key_elinfo_f] = elinfo_force
    elinfo_moment = row(text_elinfo_moment1, input_elinfo_moment, text_elinfo_moment2)
    doc.children_element_info[key_elinfo_m] = elinfo_moment
    ...
    doc.children_element_info[key_elinfo_h] = elinfo_beam
    ...
    doc.children_element_info[key_elinfo_ei] = elinfo_lineload
    ...
    doc.children_element_info[key_elinfo_dt] = elinfo_dt
    doc.layout_element_info = row(minispacer, column(minispacer, elinfo_name, elinfo_xy, elinfo_angle_length, spacer,
    ...
    user_input = row(doc.plot_input, spacer, layout_input_elements)
    user_input_info = row(doc.div_input, div_xy, spacer, button_del_selected, button_del_all)
    user_msg = row(doc.div_msg, spacer, button_calc)

    # output plots and check boxes for characteristic values
    user_output = column(
        row(doc.plot_normal_f   , spacer, column(spacer, check_plot_nf, sign_convention_N   )),
        row(doc.plot_normal_disp, spacer, column(spacer, check_plot_nd, sign_convention_u   )),
        row(doc.plot_shear_f    , spacer, column(spacer, check_plot_sf, sign_convention_SF  )),
        row(doc.plot_moment     , spacer, column(spacer, check_plot_mo, sign_convention_M   )),
        row(doc.plot_shear_angle, spacer, column(spacer, check_plot_sa, sign_convention_phi )),
        row(doc.plot_shear_disp , spacer, column(spacer, check_plot_sd, sign_convention_w   )))

    # assemble complete layout
    doc_layout = column(spacer, row(spacer, spacer, dropdown_tc),
                        row(column(user_input, user_input_info, user_msg), minispacer, doc.layout_element_info), spacer,
                        user_output)

    # add layout
    doc.doc.add_root(doc_layout)
    # set title of browser tab
    doc.doc.title = split(dirname(__file__))[-1].replace('_', ' ').replace('-', ' ')
