...
#Sliders:
p_loc_slide= Slider(title="Load Position",value= p_loci,start = x0, end = xf, step = 1/resol)
p_mag_slide = Slider(title="Load Amplitude", value=p_mag, start=-2*p_mag, end=2*p_mag, step=1)
f2_loc_slide = Slider(title="Support Position",value=f2_loci,start = x0, end = xf, step = 1/resol)
...
#####################
        ...
        #f1_arrow:
            ...
            #f1_arrow_source.data = dict(xS=[], xE=[], yS=[], yE=[], lW = [])
        if (f1_mag<=-p_magi):
            f1_arrow_source.data = dict(xS= [0], xE= [0], yS= [1-(f1_mag/200.0)], yE=[0.8], lW = [2])
        ...
        if (f2_mag<=-p_magi):
            f2_arrow_source.data = dict(xS= [f2_coord], xE= [f2_coord], yS= [1-(f2_mag/200.0)], yE=[0.8], lW = [2])
