...
def changeFunc(attr,old,new):
    ...
    if new == "rect(t)":
        ...
        mod_arrow_line_source.data = dict(x=[0],y=[0])
        ...
        mod_freq_Re_source.data = dict(x=x,y=y_FT_Re) 
        mod_freq_Im_source.data = dict(x=x, y=y_FT_Im)
        mod_arrow_source.stream(dict(x1=[100],y1=[0],x2=[100],y2=[0]),rollover=1)
        mod_arrow_source_Re.stream(dict(x1=[100],y1=[0],x2=[100],y2=[0]),rollover=1)
        ...
        mod_arrow_line_source.data = dict(x=[1,1],y=[0,-pi])
        mod_select.value = "Scaling"
        lambda_input.value = 1
    elif new == "cos(t)":
        for i in range(0,1401):
...
mod_arrow_line_source = ColumnDataSource(data=dict(x=[0],y=[0]))
...
func_freq_plot.xaxis.axis_label="t"
...
mod_freq_plot.xaxis.axis_label="t"
...
mod_freq_plot.line(x='x',y='y', source = mod_arrow_line_source, color='#a2ad00', line_dash='dashed',line_width=2.5)
