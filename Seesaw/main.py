...
def changeShow():
    ...
    if not show:
        ...
        show_button.label = "Hide values"
    else:
        value_plot.text= ""
        show_button.label = "Show values"
     
# Slider to change location of Forces F1 and F2
F1F2Location_slider = LatexSlider(title="\\text{Length } a =",value=20, start=1, end=39, step=1, value_unit="\\text{m}")
F1F2Location_slider.on_change('value',changeLength)

# Button to show forces
show_button = Button(label="Show values", button_type="success")
...
curdoc().add_root(column(description,row(plot,column(F1F2Location_slider,show_button,value_plot))))
curdoc().title = "Moment generated by two forces"
