       ...
       Amp_values.data['y'] = np.zeros(max_omega+1)     # precludes conditions, where no calculation is needed
        ...
        while j <= max_omega:
            ...
            if NAF_values.data['x'][0] <= length_left:  # for the subsystem on the left
                Y_temp[j] = -1*j*(A1*sin(lam*NAF_values.data['x'][0])+A2*cos(lam*NAF_values.data['x'][0])
                                 +A3*sinh(lam*NAF_values.data['x'][0])+A4*cosh(lam*NAF_values.data['x'][0]))
            else:                                       # for the subsystem on the right
                Y_temp[j] = -1*j*(B1*sin(lam*(NAF_values.data['x'][0]-length_left))+B2*cos(lam*(NAF_values.data['x'][0]-length_left))
                                 +B3*sinh(lam*(NAF_values.data['x'][0]-length_left))+B4*cosh(lam*(NAF_values.data['x'][0]-length_left)))
            j+=1
