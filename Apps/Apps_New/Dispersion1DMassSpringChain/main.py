...
label_dictionary = ColumnDataSource(data=dict(deutsch=[['Frequenzsperrband anzeigen', 'Frequenzsperrband ausblenden'],
                                'Visualisierung der Dynamik der Masse-Feder-Kette für den ausgewählten Normal Mode:',
                                'Visualisierung der Dynamik der Masse-Feder-Kette für die superpositionierten Normal Modes:',
                                'Visualisierung der longitudinal Verschiebungen der diskreten Massepunkte als transversale Welle',
                                ['Frequenzsperrbänder anzeigen', 'Frequenzsperrbänder ausblenden'],
                                                       'Longitudinale Verschiebung',
                                                       'Ruhelage des diskreten Massepunkts',
                                'Visualisierung der Dynamik der Masse-Feder-Kette für den ausgewählten akustischen Normal Mode:',
                                'Visualisierung der Dynamik der Masse-Feder-Kette für den ausgewählten optischen Normal Mode:',
                                'Visualisierung der Dynamik der Masse-Feder-Kette für die superpositionierten akustischen Normal Modes:',
                                'Visualisierung der Dynamik der Masse-Feder-Kette für die superpositionierten optischen Normal Modes:',
                                ...
                                'Visualization of the longitudinal displacements of the discrete mass points as a transverse wave',
...
legend_dictionary_mono = ColumnDataSource(data=dict(deutsch=["Akustischer Zweig", 'dimensionsloser Wellenvektor',
                                                        'dimensionsloser Wellenvektor 1 (2.Animation)', 'dimensionsloser Wellenvektor 2 (2.Animation)',
                                                            'Position einer Masse','Carrier-Welle','Hüllkurve der Wellenpakete',
                                                             'Phasengeschwindigkeitsvektor', 'Gruppengeschwindigkeitsvektor'
                                                             ],
                                                    english=["Acoustical Branch", 'dimensionless wavevector',
                                                        'dimensionless wavevector 1 (2.Animation)', 'dimensionless wavevector 2 (2.Animation)',
...
legend_dictionary_di = ColumnDataSource(data=dict(deutsch=["Akustischer Zweig",'Optischer Zweig', 'dimensionsloser Wellenvektor',
                                                        'dimensionsloser Wellenvektor 1 - akustische Superposition (2.Animation)',
                                                        'dimensionsloser Wellenvektor 2 - akustische Superposition (2.Animation)',
                                                        'dimensionsloser Wellenvektor 1 - optische Superposition (2.Animation)',
                                                        'dimensionsloser Wellenvektor 2 - optische Superposition (2.Animation)',
                                                    ...
                                                    english=["Acoustical Branch", "Optical Branch", 'dimensionless wavevector',
                                                        'dimensionless wavevector 1 - acoustic Superposition (2.Animation)',
                                                        'dimensionless wavevector 2 - acoustic Superposition(2.Animation)',
                                                        'dimensionless wavevector 1 - optical Superposition (2.Animation)',
                                                        'dimensionless wavevector 2 - optical Superposition(2.Animation)',
...
label_playbutton_dictionary = ColumnDataSource(data=dict(deutsch=[['Monoatomare Superposition Animation starten', 'Monoatomare Superposition Animation pausieren'],
                                                                  ['Monoatomare Normal Mode Animation starten', 'Monoatomare Normal Mode Animation pausieren'],
                                                                  ['Diatomare Normal Mode Animation starten', 'Diatomare Normal Mode Animation pausieren'],
                        ...
                        ['Start Monoatomic Normal Mode Animation', 'Pause Monoatomic Normal Mode Animation'],
                        ['Start Diatomic Normal Mode Animation', 'Pause Diatomic Normal Mode Animation'],
...
global_timer = Paragraph(text="Timer [s]:" + str(np.round(timer_1, decimals=2)),
                         styles={'font-style': 'italic', 'font-size': '14px'})
...
fig_dispersion_mono.yaxis.axis_label = r"$$\frac{\omega}{\sqrt{\frac{4c}{M}}}\$$"
fig_dispersion_mono.xaxis.axis_label = "$$\mu$$"
...
ka_mono_button_label = Paragraph(text="$$\mu$$= " + str(np.round(ka_mono, decimals=2)), styles={'font-style': 'italic', 'font-size': '16px'})
ka_di_button_label = Paragraph(text="$$\mu$$= " + str(np.round(ka_di, decimals=2)), styles={'font-style': 'italic', 'font-size': '16px'})
...
def mono_ka_plus_button_callback():
    ...
    if ka_mono + 2*np.pi/mass_anzahl <= 3.1:            # Nur die zulässigen Punkte auswählen lassen
        ...
        ka_mono = np.pi
    ka_mono_button_label.text = "$$\mu$$= " + str(np.round(ka_mono, decimals=2))    # Text aktualisieren mit neuem Wert
...
def mono_ka_minus_button_callback():
    ...
    if ka_mono - 2*np.pi/mass_anzahl >= -3.1:
        ...
        ka_mono = - np.pi
    ka_mono_button_label.text = "$$\mu$$= " + str(np.round(ka_mono, decimals=2))
    update_plot()
    reset_time()
...
# Plus Button
mono_ka_plus_button = Button(label="+", button_type="primary")
mono_ka_plus_button.on_click(mono_ka_plus_button_callback)
# Minus Button
mono_ka_minus_button = Button(label="-", button_type="primary")
mono_ka_minus_button.on_click(mono_ka_minus_button_callback)
# Bandgap Button
mono_bandgap_button = Button(label=label_dictionary.data[language][0][0], button_type='primary')
...
ka_source_mono_superpos = ColumnDataSource(data=dict(ka_1=[ka_mono_superpos1], ka_2=[ka_mono_superpos2],
                                                     ...
                                                     y_2=[dis_rel_mono(ka_mono_superpos2)]))
# ka-Anzeige anlegen
ka_mono_superpos1_button_label = Paragraph(text="$$\mu_1$$= " + str(np.round(ka_mono_superpos1, decimals=2)), styles={'font-style': 'italic', 'font-size': '16px'})
ka_mono_superpos2_button_label = Paragraph(text="$$\mu_2$$= " + str(np.round(ka_mono_superpos2, decimals=2)), styles={'font-style': 'italic', 'font-size': '16px'})
...
def mono_superpos1_ka_plus_button_callback():
    ...
    ka_mono_superpos1_button_label.text = "$$\mu_1$$= " + str(np.round(ka_mono_superpos1, decimals=2))
...
def mono_superpos2_ka_plus_button_callback():
    ...
    ka_mono_superpos2_button_label.text = "$$\mu_2$$= " + str(np.round(ka_mono_superpos2, decimals=2))
    update_plot()       # ka-Punkte aktualisieren
    reset_time()
...
# Plus Button
mono_superpos1_ka_plus_button = Button(label="+", button_type="primary")
mono_superpos1_ka_plus_button.on_click(mono_superpos1_ka_plus_button_callback)

mono_superpos2_ka_plus_button = Button(label="+", button_type="primary")
mono_superpos2_ka_plus_button.on_click(mono_superpos2_ka_plus_button_callback)
# Minus Button
mono_superpos1_ka_minus_button = Button(label="-", button_type="primary")
mono_superpos1_ka_minus_button.on_click(mono_superpos1_ka_minus_button_callback)

mono_superpos2_ka_minus_button = Button(label="-", button_type="primary")
...
# ------------- Mass Button
mass_button_label = Paragraph(text=r"$$\frac{M_1}{M_2}$$= " + str(mass_relation))
...
def di_ka_plus_button_callback():
    ...
    if ka_di + 2*np.pi/unitcell_number <= 3.1:
        ...
        ka_di = np.pi
    ka_di_button_label.text = "$$\mu$$= " + str(np.round(ka_di, decimals=2))
    update_plot()
    reset_time()
...
def update_diatomic_plot():
    mass_button_label.text = r"$$\frac{M_1}{M_2}$$=" + str(np.round(mass_relation, decimals=2))
...
# Ka Buttons
# Plus Button
di_ka_plus_button = Button(label="+", button_type="primary")
di_ka_plus_button.on_click(di_ka_plus_button_callback)
# Minus Button
di_ka_minus_button = Button(label="-", button_type="primary")
di_ka_minus_button.on_click(di_ka_minus_button_callback)

# Mass Button
# Plus Button
mass_plus_button = Button(label="+", button_type="primary")
mass_plus_button.on_click(mass_plus_button_callback)
# Minus Button
mass_minus_button = Button(label="-", button_type="primary")
mass_minus_button.on_click(mass_minus_button_callback)

# Band Gap Buttons
di_bandgap_button = Button(label=label_dictionary.data[language][4][0], button_type='primary')
...
fig_dispersion_di.yaxis.axis_label = r"$$\frac{\omega}{\sqrt{2c*(\frac{1}{M_1} + \frac{1}{M_2})}}\$$"
fig_dispersion_di.xaxis.axis_label = "$$\mu$$"
...
ka_di_acou_superpos1_button_label = \
    Paragraph(text="$$\mu_1$$= " + str(np.round(ka_di_acou_superpos1, decimals=2)), styles={'font-style': 'italic', 'font-size': '16px'})
ka_di_acou_superpos2_button_label = \
    Paragraph(text="$$\mu_2$$= " + str(np.round(ka_di_acou_superpos2, decimals=2)), styles={'font-style': 'italic', 'font-size': '16px'})
...
def di_acou_superpos1_ka_plus_button_callback():
    ...
    ka_di_acou_superpos1_button_label.text = "$$\mu_1$$= " + str(np.round(ka_di_acou_superpos1, decimals=2))
    update_plot()
    reset_time()
...
def di_acou_superpos2_ka_plus_button_callback():
    ...
    ka_di_acou_superpos2_button_label.text = "$$\mu_2$$= " + str(np.round(ka_di_acou_superpos2, decimals=2))
    update_plot()
    reset_time()
...
# Plus Button
di_acou_superpos1_ka_plus_button = Button(label="+", button_type="primary")
di_acou_superpos1_ka_plus_button.on_click(di_acou_superpos1_ka_plus_button_callback)

di_acou_superpos2_ka_plus_button = Button(label="+", button_type="primary")
di_acou_superpos2_ka_plus_button.on_click(di_acou_superpos2_ka_plus_button_callback)

# Minus Button
di_acou_superpos1_ka_minus_button = Button(label="-", button_type="primary")
di_acou_superpos1_ka_minus_button.on_click(di_acou_superpos1_ka_minus_button_callback)

di_acou_superpos2_ka_minus_button = Button(label="-", button_type="primary")
...
diatomic_acou_superpos_animation = figure(title=label_dictionary.data[language][9], tools="",
                                       x_range=(-2, chain_length-2), y_range=(-2, 2), width=1200, height=55, toolbar_location=None)
...
displacement_over_y_di_acou = figure(title=label_dictionary.data[language][3], tools="", x_range=(0, chain_length),
                              y_range=(-1.1*mass_abstand, 1.1*mass_abstand), width=1200, height=200, toolbar_location=None)
...
ka_di_opti_superpos1_button_label = \
    Paragraph(text="$$\mu_1$$= " + str(np.round(ka_di_opti_superpos1, decimals=2)), styles={'font-style': 'italic', 'font-size': '16px'})
ka_di_opti_superpos2_button_label = \
    Paragraph(text="$$\mu_2$$= " + str(np.round(ka_di_opti_superpos2, decimals=2)), styles={'font-style': 'italic', 'font-size': '16px'})
...
def di_opti_superpos1_ka_plus_button_callback():
    ...
    ka_di_opti_superpos1_button_label.text = "$$\mu_1$$= " + str(np.round(ka_di_opti_superpos1, decimals=2))
    update_plot()
    reset_time()
...
def di_opti_superpos2_ka_plus_button_callback():
    ...
    ka_di_opti_superpos2_button_label.text = "$$\mu_2$$= " + str(np.round(ka_di_opti_superpos2, decimals=2))
    update_plot()
    reset_time()
...
# Plus Button
di_opti_superpos1_ka_plus_button = Button(label="+", button_type="primary")
di_opti_superpos1_ka_plus_button.on_click(di_opti_superpos1_ka_plus_button_callback)

di_opti_superpos2_ka_plus_button = Button(label="+", button_type="primary")
di_opti_superpos2_ka_plus_button.on_click(di_opti_superpos2_ka_plus_button_callback)
# Minus Button
di_opti_superpos1_ka_minus_button = Button(label="-", button_type="primary")
di_opti_superpos1_ka_minus_button.on_click(di_opti_superpos1_ka_minus_button_callback)

di_opti_superpos2_ka_minus_button = Button(label="-", button_type="primary")
...
diatomic_opti_superpos_animation = figure(title=label_dictionary.data[language][10], tools="",
                                       x_range=(-2, chain_length-2), y_range=(-2, 2), width=1200, height=55, toolbar_location=None)
...
displacement_over_y_di_opti = figure(title=label_dictionary.data[language][3], tools="", x_range=(0, 120),
                              y_range=(-1.1*mass_abstand, 1.1*mass_abstand), width=1200, height=200, toolbar_location=None)
...
def simulate_diatomic_opti_superposition_animation():
    ...
    x_heavy, y_heavy, x_light, y_light, y_carrier_disp_v_l, y_carrier_disp_u_l, y_envelope_disp,  v_phase_xpos,\
    v_phase_ypos, v_group = superposition_di_opti_animation.superposition_two_optical_normalmodes(
...
playbutton_mono_superpos = Button(label=label_playbutton_dictionary.data[language][0][0], button_type='primary')
...
def play_pause_mono():
    ...
    if status:
        ...
        callback_id_mono['id'] = curdoc().add_periodic_callback(simulate_monoatomic, 20)
...
playbutton_mono = Button(label=label_playbutton_dictionary.data[language][1][0], button_type='primary')
...
def play_pause_diatomic():
    ...
    if status:
        callback_id_diatomic['id'] = curdoc().add_periodic_callback(simulate_diatomic, 20)
...
playbutton_diatomic = Button(label=label_playbutton_dictionary.data[language][2][0], button_type='primary')
...
######################################
general_app_infos = Div(render_as_text=False, width=1000, height=270)
general_monoatomic_infos = Div(render_as_text=False, width=1000, height=630)
monoatomic_animation_info = Div(render_as_text=False, width=1000, height=450)
mono_superposition_info = Div(render_as_text=False, width=1000, height=660)
superposition_explained = Div(render_as_text=False, width=1000, height=100)

######################################
# Diatomic Divs vordefinieren
######################################
general_app_infos_diatomic = Div(render_as_text=False, width=1000, height=400)
general_diatomic_infos = Div(render_as_text=False, width=1000, height=930)
diatomic_animation_info = Div(render_as_text=False, width=1000, height=810)
di_superposition_info = Div(render_as_text=False, width=1000, height=720)
di_superposition_acou_explained = Div(render_as_text=False, width=1000, height=150)
di_superposition_opti_explained = Div(render_as_text=False, width=1000, height=150)
...
def update_labels():
    ...
    diatomic_optical_animation.title.text = label_dictionary.data[language][7]
...
def change_content():
    ...
    if language == 'deutsch':
        ...
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"deutsch"/"mono_beschreibung.html"), 'r') as file:
            mono_html_content1 = file.read()
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"deutsch"/"ÜberschriftIntro.html"), 'r') as file:
            mono_html_content11 = file.read()
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"deutsch"/"monoatomareAnimationsbeschreibung.html"), 'r') as file:
            mono_html_content2 = file.read()
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"deutsch"/"superposition_beschreibung.html"), 'r') as file:
            mono_html_content3 = file.read()
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"deutsch"/"superposition_erklärung.html"), 'r') as file:
            mono_html_content4 = file.read()
        # Diatomare HTMLs
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "deutsch" / "diatomar_beschreibung.html"), 'r') as file:
            di_html_content1 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "deutsch" / "diatomareAnimationbeschreibung.html"), 'r') as file:
            di_html_content2 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "deutsch" / "di_superposition_beschreibung.html"), 'r') as file:
            di_html_content3 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "deutsch" / "di_superposition_acou_erklärung.html"), 'r') as file:
            di_html_content4 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "deutsch" / "di_superposition_opti_erklärung.html"), 'r') as file:
            di_html_content5 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "deutsch" / "ÜberschriftIntro.html"), 'r') as file:
            di_html_content11 = file.read()

    elif language == 'english':
        # Monoatomic HTMLs
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"english"/"mono_beschreibung.html"), 'r') as file:
            mono_html_content1 = file.read()
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"english"/"ÜberschriftIntro.html"), 'r') as file:
            mono_html_content11 = file.read()
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"english"/"monoatomareAnimationsbeschreibung.html"), 'r') as file:
            mono_html_content2 = file.read()
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"english"/"superposition_beschreibung.html"), 'r') as file:
            mono_html_content3 = file.read()
        with open(str(app_base_path / "static"/"Html"/"monoatomic"/"english"/"superposition_erklärung.html"), 'r') as file:
            mono_html_content4 = file.read()
        # Diatomic HTMLs
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "english" / "diatomar_beschreibung.html"), 'r') as file:
            di_html_content1 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "english" / "diatomareAnimationbeschreibung.html"), 'r') as file:
            di_html_content2 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "english" / "di_superposition_beschreibung.html"), 'r') as file:
            di_html_content3 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "english" / "di_superposition_acou_erklärung.html"), 'r') as file:
            di_html_content4 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "english" / "di_superposition_opti_erklärung.html"), 'r') as file:
            di_html_content5 = file.read()
        with open(str(
            app_base_path / "static" / "Html" / "diatomic" / "english" / "ÜberschriftIntro.html"), 'r') as file:
            di_html_content11 = file.read()
    ...
    # Diatomic
    general_app_infos_diatomic.text = di_html_content11
    general_diatomic_infos.text = di_html_content1
    diatomic_animation_info.text = di_html_content2
    di_superposition_info.text = di_html_content3
    di_superposition_acou_explained.text = di_html_content4
    di_superposition_opti_explained.text = di_html_content5


# Button zum Sprachen wechseln festlegen
language_button = Button(label="Zu deutsch wechseln", button_type="success")
...
monoatomicPNG.add_glyph(monoatomicpictureSource, ImageURL(url='img_url', x=0, y=342, w=549, h=342))
...
monoatomicunitcellPNG_de.add_glyph(monoatomiceinheitszelleSource, ImageURL(url='img_url', x=0, y=150, w=150, h=150))
...
monoatomicunitcellPNG_en.add_glyph(monoatomicunitcellSource, ImageURL(url='img_url', x=0, y=150, w=150, h=150))
...
diatomicPNG.add_glyph(diatomicpictureSource, ImageURL(url='img_url', x=0, y=342, w=549, h=342))
...
diatomicunitcellPNG_de.add_glyph(diatomiceinheitszelleSource, ImageURL(url='img_url', x=0, y=140, w=283, h=140))
...
diatomicunitcellPNG_en.add_glyph(diatomicunitcellSource, ImageURL(url='img_url', x=0, y=140, w=283, h=140))
...
ka_buttons_mono_layout = row(Spacer(width=15), column(row(Spacer(width=10), ka_mono_button_label), Spacer(height=1),
                                                      row(mono_ka_minus_button, Spacer(width=mono_ka_plus_button.width), mono_ka_plus_button)))
...
mono_block1 = column(general_app_infos, column(row(Spacer(width=200), monoatomicPNG), mono_unitcellPNG), general_monoatomic_infos)
...
ka_buttons1_di_acou_superpos_layout = row(Spacer(width=15), column(row(Spacer(width=10), ka_di_acou_superpos1_button_label),
                                                               Spacer(height=1), row(di_acou_superpos1_ka_minus_button,
                                                                                     Spacer(width=di_acou_superpos1_ka_plus_button.width),
...
ka_buttons2_di_acou_superpos_layout = row(Spacer(width=15), column(row(Spacer(width=10), ka_di_acou_superpos2_button_label),
                                                               Spacer(height=1), row(di_acou_superpos2_ka_minus_button,
                                                                                     Spacer(width=di_acou_superpos2_ka_plus_button.width),
...
ka_buttons1_di_opti_superpos_layout = row(Spacer(width=15), column(row(Spacer(width=10), ka_di_opti_superpos1_button_label),
                                                               Spacer(height=1), row(di_opti_superpos1_ka_minus_button,
                                                                                     Spacer(width=di_opti_superpos1_ka_plus_button.width),
...
ka_buttons2_di_opti_superpos_layout = row(Spacer(width=15), column(row(Spacer(width=10), ka_di_opti_superpos2_button_label),
                                                               Spacer(height=1), row(di_opti_superpos2_ka_minus_button,
                                                                                     Spacer(width=di_opti_superpos2_ka_plus_button.width),
...
ka_buttons_di_layout = row(Spacer(width=15), column(row(Spacer(width=10), ka_di_button_label), Spacer(height=1),
                                                    row(di_ka_minus_button, Spacer(width=di_ka_plus_button.width),
                                                        di_ka_plus_button)))
mass_buttons_di_layout = row(Spacer(width=15), column(row(Spacer(width=10), mass_button_label), Spacer(height=1),
                                                    row(mass_minus_button, Spacer(width=mass_plus_button.width),
                                                        mass_plus_button)))
...
# Definition der einzelnen Layout Teile
di_block1 = column(general_app_infos_diatomic, column(row(Spacer(width=200), diatomicPNG), di_unitcellPNG), general_diatomic_infos)
...
##################
menu_selection = Button(label="Diatomic Mass-Spring-Chain", button_type="primary")
