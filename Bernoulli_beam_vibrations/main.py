...
support_righta=ColumnDataSource(data=dict(x=[1,0.95,1.05,1],y=[0,-0.4,-0.4,0]))
support_rightb= ColumnDataSource(data=dict(x=[0.95,1.05,1.025,1.05,1.025,1,1.025,1,0.975,1,0.975,0.95,0.975,0.95],y=[-0.5,-0.5,-0.7,-0.5,-0.5,-0.7,-0.5,-0.5,
...
fixed_left=ColumnDataSource(data=dict(x=[0,0,-0.03,0,0,-0.03,0,0,-0.03,0,0,-0.03,0,0,-0.03],y=[0.25,-0.25,-0.3,-0.25,-0.15,-0.175,0.35,-0.25,-0.05,-0,0.125,
                                      0.005,0.125,0.2,0.2]))
fixed_right=ColumnDataSource(data=dict(x=[1,1,1.03,1,1,1.03,1,1,1.03,1,1,1.03,1,1,1.03],y=[-0.25,-2.75,-2.7,-2.75,-2.625,-2.575,-2.625,-2.5,-2.45,-2.5,-2.375,-2.325,
                                       -2.375,-2.25,-2.2]))
...
y3=-(1/2*(np.cosh(1.8751*x3/l)-np.cos(1.8751*x3/l))-((1/2*(np.cosh(1.8751)+np.cos(1.8751)))/(1/2*(np.sinh(1.8751)+np.sin(1.8751))))*1/2*(np.sinh(1.8751*x3/l)-
            np.sin(1.8751*x3/l)))
y3cosh=-(1/2*(np.cosh(4.73004*x2/l)))
y3sinh=+((1/2*(np.cosh(4.73004)+np.cos(4.73004)))/(1/2*(np.sinh(4.73004)+np.sin(4.73004))))*1/2*(np.sinh(4.73004*x2/l))
y3cos=-(1/2*(np.cos(4.73004*x2/l)))
y3sin=-((1/2*(np.cosh(4.73004)+np.cos(4.73004)))/(1/2*(np.sinh(4.73004)+np.sin(4.73004))))*1/2*(np.sin(4.73004*x2/l))
...
#Plot 1
p1 = figure(plot_height=250, plot_width=800,title="Simply supported beam", tools="", x_range=(-0.075,1.5), y_range=(-2.5,2.5))
...
legend1 = Legend(items=[
    ("Eigenvalue Problem: sin("u"\u03BB)=0 / Solution: w"u"\u1D62("u"\u03BE)=sin(i"u"\u00B7"u"\u03C0"u"\u00B7"u"\u03BE)"   , [eigenmodes_beam1]),
], location=(0, 0))
...
#Plot 2
p2 = figure(plot_height=400, plot_width=800,title="Beam fixed at both ends", tools="", x_range=(-0.075,1.5), y_range=(-3,3))
...
eigenmodes_beam2cosh=p2.line(x='x', y='y', source=source2cosh,
                         line_width=3, color='#c6f808')#greeny wellow
eigenmodes_beam2sinh=p2.line(x='x', y='y', source=source2sinh,
                         line_width=3,line_color='#9bb53c')#booger
eigenmodes_beam2cos=p2.line(x='x', y='y', source=source2cos,
                         line_width=3,line_color='#677a04')#olive green
eigenmodes_beam2sin=p2.line(x='x', y='y', source=source2sin,
                         line_width=3,line_color='#ffda03')#sunflower yellow
fixed_support_left=p2.line(x='x', y='y', source=fixed_left,line_width=1,line_color='black')
fixed_support_right=p2.line(x='x', y='y', source=fixed_right,line_width=1,line_color='black')
legend2 = Legend(items=[
    ("Eigenvalue Problem: cosh("u"\u03BB)"u"\u00B7cos("u"\u03BB)-1=0 / Solution: w"u"\u1D62("u"\u03BE) = c("u"\u03BB"u"\u1D62"u"\u03BE) - c("u"\u03BB"u"\u1D62) / s("u"\u03BB"u"\u1D62) "u"\u00B7s ("u"\u03BB"u"\u1D62"u"\u03BE)", [eigenmodes_beam2]),
...
#Plot 3
p3 = figure(plot_height=400, plot_width=800,title="Cantilever beam", tools="", x_range=(-0.075,1.5), y_range=(-3,3))
...
fixed_cantilever=p3.line(x='x', y='y', source=fixed_cantil,line_width=1,line_color='black')
...
eigenmodes_beam3cosh=p3.line(x='x', y='y', source=source3cosh,
                         line_width=3, color='#fd8d49')#orangeish
eigenmodes_beam3sinh=p3.line(x='x', y='y', source=source3sinh,
                         line_width=3,line_color='#c45508')#rust orange
eigenmodes_beam3cos=p3.line(x='x', y='y', source=source3cos,
                         line_width=3,line_color='#fe4b03')#blood orange
eigenmodes_beam3sin=p3.line(x='x', y='y', source=source3sin,
                         line_width=3,line_color='#fdaa48')#ligth orange
#eigenmodes_beam3.visible= False
legend3 = Legend(items=[
    ("Eigenvalue Problem: cosh("u"\u03BB)"u"\u00B7cos("u"\u03BB)+1=0 / Solution: w"u"\u1D62("u"\u03BE) = c("u"\u03BB"u"\u1D62"u"\u03BE) - C("u"\u03BB"u"\u1D62) / S("u"\u03BB"u"\u1D62) "u"\u00B7s ("u"\u03BB"u"\u1D62"u"\u03BE)", [eigenmodes_beam3]),
...
p4 = figure(plot_height=400, plot_width=300,title="Definitions", tools="", x_range=(-1,1), y_range=(-1,1))
...
def update_data(attrname, old, new):
    ...
    if new==1:
        ...
        y3=-(1/2*(np.cosh(4.69409*x3/l)-np.cos(4.69409*x3/l))-((1/2*(np.cosh(4.69409)+np.cos(4.69409)))/(1/2*(np.sinh(4.69409)+np.sin(4.69409))))*1/2*(np.sinh(4.69409*x3/l)-np.sin(4.69409*x3/l)))
        source3.data=dict(x=x3,y=y3)
    elif new==3:
...
curdoc().add_root(row(column(p1,p2,p3),column(toggle1,toggle2,toggle3,Eigenvalue_input,p4)))
